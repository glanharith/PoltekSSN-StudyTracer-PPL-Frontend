...
import { FormEditorInput, FormEditorProps } from './interface';
...
export const FormEditor = ({ existingData, id }: FormEditorProps) => {
  ...
  const methods = useForm<FormEditorInput>({
  ...
  const { fields: updateQuestionFields, remove: removeUpdateQuestion } =
    useFieldArray({
  ...
  const { append: deleteExistingQuestion } = useFieldArray({
  ...
  const onSubmit = async (data: FormEditorInput) => {
    ...
    const dataWithOrder = {
      ...
      updateQuestions: data.updateQuestions.map((q) => {
        return {
          ...q,
          newOptions: (q.newOptions ?? []).map((o, optionIdx) => {
            return {
              ...o,
              order: optionIdx + (q.updateOptions ? q.updateOptions.length : 0),
            };
          }),
          deleteOptions: q.deleteOptions ?? [],
        };
      }),

      newQuestions: data.newQuestions.map((q, questionIdx) => {
        const options = q.newOptions?.map((o, optionIdx) => {
          return {
            ...o,
            order: optionIdx + (q.updateOptions ? q.updateOptions.length : 0),
          };
        ...
        delete q.newOptions;
        delete q.updateOptions;
        delete q.deleteOptions;

        return {
          ...q,
          order: 1 + questionIdx + data.updateQuestions.length,
          options: options,
        };
      }),
    };
    ...
    >
      <FormProvider {...methods}>
        ...
        <section className="flex flex-col gap-6 p-4 rounded-xl bg-white items-center">
          {updateQuestionFields.map((q, idx) => (
            <Flex direction="column" w="full" gap={6} key={idx}>
              <QuestionInput
                ...
                index={idx}
                remove={() => {
                  if (q.id) deleteExistingQuestion({ id: q.id });
                  removeUpdateQuestion(idx);
                }}
              />
              {idx < updateQuestionFields.length - 1 && (
                <Divider key={idx} borderColor="gray" borderWidth={1.5} />
              )}
            </Flex>
          ))}

          {newQuestionFields.length > 0 && (
            <Divider borderColor="gray" borderWidth={1.5} />
          )}

          {newQuestionFields.map((q, idx) => (
            <Flex direction="column" w="full" gap={6} key={idx}>
              <QuestionInput
                method="new"
                key={q.uid}
              ...
              />
              {idx < newQuestionFields.length - 1 && (
                <Divider key={idx} borderColor="gray" borderWidth={1.5} />
              )}
            </Flex>
          ))}
